...
+ (Physics *) physics {
  return [[[Physics alloc] init] autorelease];
}

- (id) init {
  if(self = [super init]) {
    self.forces = [[[NSMutableArray alloc] initWithCapacity:100] autorelease];
    self.forcebank = [[[Bank alloc] initWithType:[Force class]] autorelease];
  }
  return self;
}
...
- (Entity *) firstEntity:(NSArray *) entities touching:(Entity *) t {
...
- (Collision *) collisionWith:(Entity *) a and: (Entity *) b {
  Collision *collision = [[[Collision alloc] init] autorelease];
